...
QtObject {
    ...
    function calculateTooltipPosition(widget, tooltipWidth, tooltipHeight) {
        if (!tooltipWidth)
            tooltipWidth = 200;  // Default width
        if (!tooltipHeight)
            tooltipHeight = 100;  // Default height
    ...
    function getDebugInfo(widget) {
        ...
        return "Screen: " + screen.width + "x" + screen.height + ", Widget: " + widgetGlobalPos.x + "," + widgetGlobalPos.y + ", Panel: " + panelWindow.x + "," + panelWindow.y + " " + panelWindow.width + "x" + panelWindow.height;
    }
}
